...
testSkipPaths = prop "test that skip creates valid paths"
                $ \k -> generateProgramPaths k Skip == [[]]

testZeroK = prop "test that k=0 always returns no paths if the statement is not Skip"
...
testNotTooLong = prop "test that it doesn't generate too long paths"
                 $ \stmt -> do
                   k <- choose (0, 8)
                   return $ all (\x -> length x <= k)
                     $ generateProgramPaths k stmt

testCombinesIfThenElse = prop "test that as many program paths are generated for if then else as for both branches added together"
                         $ \s1 s2 -> do
                           k <- choose (1, 4)
...
testIgnoresEmptyBlocks = prop "test that empty blocks are ignored" $
                    \s -> do
                      k <- choose (0, 5)
                      return $ generateProgramPaths k s
                        == generateProgramPaths k (Block [] s)

testSeqMakesCartesianProduct = prop "test that seq creates all combinations"
                               $ \s2 -> do
                                 k <- choose (0, 8)
...
testWhileContainsEndCase = prop "test that while contains the end case"
                           $ \s -> do
                             k <- choose (1, 8)
